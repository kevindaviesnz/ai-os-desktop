            ...
            if (*devid == VIRTIO_DEV_INPUT) {
                ...
                *status = VIRTIO_STATUS_ACKNOWLEDGE | VIRTIO_STATUS_DRIVER;
                ...
                if (*version == 1) {
                    ...
                    *q_sel = 0;
                    *q_num = VIRTQ_SIZE;
                    *q_desc_low = (uint32_t)(uint64_t)gpu_desc;   *q_desc_high = 0;
                    *q_avail_low = (uint32_t)(uint64_t)gpu_avail; *q_avail_high = 0;
                    *q_used_low = (uint32_t)(uint64_t)gpu_used;   *q_used_high = 0;
                    *q_ready = 1;
